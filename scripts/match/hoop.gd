...
## In hoop.png the rim center is pixel (128, 180) and the floor is y=480 —
## keep that alignment if you replace the art.

const RIM_PX := Vector2(128.0, 180.0)
const ART_SIZE := Vector2(192.0, 480.0)
...
func setup(team_attacking: int) -> void:
	...
	position = CourtGeometry.project(CourtGeometry.hoop_pos(team_attacking))
